...
test("stringToBase64Url() encodes string to base64URL", () => {
  for (const [input, expected] of testSetBase64Url) {
    const actual = stringToBase64Url(input);
    strictEqual(
      actual,
      expected,
...
test("base64UrlToString() encodes string to base64URL", () => {
  for (const [expected, input] of testSetBase64Url) {
    const actual = base64UrlToString(input);
    strictEqual(
      actual,
      expected,
      `Failed to encode "${input}" to "${expected}". Got: "${actual}"`,
